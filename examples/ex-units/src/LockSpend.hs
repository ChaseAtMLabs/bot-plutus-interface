module LockSpend (lockThenSpend, timeDebug) where
...
import Ledger
  ( Address,
    ...
    unitDatum,
    unitRedeemer,
    validatorHash, POSIXTime (POSIXTime), Interval (Interval), to, interval
  )
...
timeDebug ::  Contract () EmptySchema Text String
timeDebug = do
  timeWatitngFrom <- Contract.currentTime
  let timeWaitingTo = timeWatitngFrom + POSIXTime 5_000
  waited <- Contract.awaitTime timeWaitingTo

  let res =      "Wated  from: " ++ show timeWatitngFrom
            ++ "\nWaited   to: " ++ show timeWaitingTo
            ++ "\nWaited real: " ++ show waited
  let 
      constr =
        Constraints.mustPayToOtherScript
          ...
          unitDatum
          (Value.adaValueOf 10)
        <> Constraints.mustValidateIn (interval timeWatitngFrom (timeWaitingTo - 3_000))
  !tx <- submitTx constr
  ...
  where
    spendUtxo oref1 utxos1 oref2 utxos2 = do
      ...
      let token = Value.singleton currencySymbol (tokenName "ff") 1
          token21 = Value.singleton currencySymbol2 (tokenName "aaa") 2
          token22 = Value.singleton currencySymbol2 (tokenName "zzz") 3
          txc1 =
            Constraints.mustSpendScriptOutput oref1 unitRedeemer
              <> Constraints.mustMintValueWithRedeemer unitRedeemer token
              <> Constraints.mustMintValueWithRedeemer unitRedeemer token21
              <> Constraints.mustMintValueWithRedeemer unitRedeemer token22
              <> Constraints.mustValidateIn (to $ timeWatitngFrom + 0)
              -- <> Constraints.mustBeSignedBy ownPkh
          lookups1 =
            Constraints.unspentOutputs (Map.fromList utxos1)
              <> Constraints.otherScript validator
              <> Constraints.mintingPolicy mintingPolicy
              <> Constraints.mintingPolicy mintingPolicy2
...
mkPolicy _ !ctx =
  ...
  where
    ...
    check =
      PP.length (flattenValue PP.$ txInfoMint info) PP.== 3
...
-- minting policy2
{-# INLINEABLE mkPolicy2 #-}
mkPolicy2 :: () -> ScriptContext -> Bool
mkPolicy2 _ !ctx =
  PP.traceIfFalse "Let me mint2" check
  where
    check = 
      PP.length someWork PP.== 2

    someWork = PP.sort [1, 0] :: [Integer]

mintingPolicy2 :: MintingPolicy
mintingPolicy2 =
  mkMintingPolicyScript
    $$(PlutusTx.compile [||wrapMintingPolicy mkPolicy2||])

currencySymbol2 :: CurrencySymbol
currencySymbol2 = scriptCurrencySymbol mintingPolicy2
